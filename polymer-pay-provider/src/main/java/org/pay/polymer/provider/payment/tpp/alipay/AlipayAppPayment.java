 ...
 * @since 2019-03-05
 */
@Service(group = ALI_APP)
public class AlipayAppPayment implements AlipayPayment {

    private static Logger logger = LoggerFactory.getLogger(AlipayAppPayment.class);


    @Override
    @PaymentLogMnager(group = ALI_PAY, type = ALI_TYPE_APP, module = ALI_MODULE_PAY)
    public String doPay(PayMent doPay) {
        ...
        try {
            //调用SDK生成表单
            return JSONObject.parseObject(alipayClient.pageExecute(alipayRequest).getBody())
                    .getString(ALIPAY_TRADE_APP_PAY_RESPONSE);
        } catch (AlipayApiException e) {
            logger.error("Polymer^_^Alipay^_^App^_^Pay^_^Error^_^{}", e);
        }
        return null;
    }
}
