 ...
 */
public class UnionpayLogger {

    private static Logger clog = LoggerFactory.getLogger(UnionpayLogger.class);
    private static Logger blog = LoggerFactory.getLogger(UnionpayLogger.class);
    ...
    public static void setNeedEnableLogger(boolean needEnableLogger) {
        UnionpayLogger.needEnableLogger = needEnableLogger;
    }
    ...
    private static void logCommError(Exception e, HttpURLConnection conn, String url,
        ...
        DateFormat df = new SimpleDateFormat(UnionpayConstants.DATE_TIME_FORMAT);
        df.setTimeZone(TimeZone.getTimeZone(UnionpayConstants.DATE_TIMEZONE));
        String sdkName = "Unionpay5.1.0";
        String urlStr = null;
        String rspCode = "";
        if (conn != null) {
            try {
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logErrorScene(String resultXml, UnionpayResponse tRsp,
                                     long reqTime) {
        if (!needEnableLogger) {
            return;
        }
        ...
        appendLog((UnionpayHashMap) tRsp.getParams(), sb);
        sb.append("^_^");
        sb.append(reqTime);
        sb.append("ms");
        blog.error(sb.toString());
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logBizSummary(String resultXmlData, UnionpayResponse tRsp,
                                     long reqTime) {
        if (!needEnableLogger) {
            return;
        }
    ...
    private static void appendLog(UnionpayHashMap map, StringBuilder sb) {
        boolean first = true;
